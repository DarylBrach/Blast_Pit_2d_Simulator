...
Hosted CI `codex.cmd`/Docker `FileNotFoundError` means governed-host live binding ran in the wrong tier; do not synthesize identities. `Microsoft.PowerShell.Security` load failure means ACL testing inherited an incompatible `PSModulePath`; use the sanitized runtime-equivalent environment. The draft PR must retain a passing GitHub `tests` check before merge.
