...
Runner `20260712T005650Z` failed before creating a controller bundle with `unsafe stale worktree entry ... .controller-v2.lock`. This was the controller's own active fixed kernel-lock path, not stale candidate material. The scanner now exempts only `.controller-v2.lock`; other unexpected worktree-root entries still fail closed. Do not delete an active lock to bypass the check. Corrected exact-entrypoint DryRun evidence is runner `20260712T005801Z` / controller `20260712T005807Z-709f67a4`.

`runner publication recovery is pending`: preserve the WAL, run directory, terminal ledger, binding ledger, pointer, and advisory state; restart through the same launcher for locked startup replay. Never truncate ledgers or delete the WAL. Malformed/hash-mismatched/non-tip WAL fails closed. `runner name is already bound`: choose a new stable name; never edit the binding ledger. `_state.json` may show live parallel work and is not terminal authority; use `_terminal.json` and its ledger bindings.

For the current supported DryRun, Status/Review exit 1 is expected rather than a failure: controller `20260712T014742Z-2a8b7152` is a verified `DRY_RUN_COMPLETE` record with execution/review/promotion `NOT_RUN` and rubric 0/100. Verify and Recover exit 0. Do not alter the decision to force review readiness.

Hosted CI `codex.cmd`/Docker `FileNotFoundError` means governed-host live binding ran in the wrong tier; do not synthesize identities. A root-binding mismatch means the portable static test used checkout-derived external roots instead of the authorization's approved roots. An image-build-input mismatch on Windows CI requires checking checkout line endings: only CRLF-to-LF transport is accepted; BOM or bare CR is a hard failure, and the governed host remains raw-byte exact. `Microsoft.PowerShell.Security` load failure means ACL testing inherited an incompatible `PSModulePath`; use the sanitized runtime-equivalent environment. An exact-SDDL capability skip means the CI filesystem normalized the descriptor after `Set-Acl`; retain the skip and require exact integration on the governed host. The draft PR must retain a passing GitHub `tests` check before merge.

An ACL-helper pytest timeout after `powershell.exe` has already exited indicates a descendant retained a captured pipe handle. Invoke helper tests through the production bounded Job Object runner; do not replace confinement assertions with skips or unbounded waits.

Live Docker integration tests serialize pytest processes with a named daemon-scoped mutex and check only their unique run labels. Do not run them beside an active improvement controller: the test mutex is not controller authority. Stop the controller, verify governed-container quiescence, then retry; never make global-empty assertions against unrelated governed runs.
