...
Final local qualification is 241 passed and 3 skipped in 54.65 seconds. The skips are governed-host live binding opt-in, unavailable file symlink creation, and unavailable directory symlink creation. Exact live `load_authorization` separately passes 1/1 with `BLAST_PIT_RUN_GOVERNED_HOST_TESTS=1`. Hosted CI runs mandatory static authorization plus Windows-native ACL coverage in a sanitized PowerShell environment; it never synthesizes bound Codex/Docker/image identity. Focused runner remains 27 passed. Exact-entrypoint DryRun runner `20260712T014740Z-04461e70` / controller `20260712T014742Z-2a8b7152` passes retained verification. The draft PR must retain a passing GitHub `tests` check before merge.
