...
Runner v3.1 startup holds the publication kernel lock, recovers a valid WAL, validates the hash-chain name-binding ledger, then allocates a run. Never edit/delete the WAL, terminal ledger, binding ledger, pointer, or locks. `_state.json` is live/advisory; completion requires `_terminal.json`, manifest v2, terminal-ledger verification, and matching `name_binding_hash`. Process-crash recovery is qualified, not sudden-power-loss recovery. A post-v3.1 supported DryRun remains pending.
