...
Ten cycles means up to ten attempts before the deadline. `10/10` means 100/100 and only readiness for human review. Current code qualification is 240 passed with 2 platform skips. Supported historical DryRun qualification is runner `20260712T005801Z` / controller `20260712T005807Z-709f67a4`; DryRun does not execute candidate build, tests, training, scoring, panel, or final QA. First live run `20260711T223022Z-1128b831` remains unanchored diagnostic evidence.

## Controller v2.2 Docker operation

Before Run/DryRun, Docker host identity, `candidate_image_attestation_v1.json`, the hash-locked recipe, and image `sha256:ebecafb90288df12553cb8b66e0bc2a3ce325513a19f65e40e5ce9e526db0698` must match authorization. Builds are reviewed maintenance; runs use pull-never. Compile/tests/train/trusted score are containerized with a read-only workspace, bounded 256 MiB `/tmp` and `/output`, and trusted stdout export only. Recover removes/verifies containers first, then restores ACL leases. Post-v3.1 runner `20260712T014740Z-04461e70` and controller `20260712T014742Z-2a8b7152` are the current supported DryRun record; Recover and Verify returned 0, while Status/Review correctly returned 1 because DryRun is not review-ready.
...
Runner v3.1 startup holds the publication kernel lock, recovers a valid WAL, validates the hash-chain name-binding ledger, then allocates a run. Never edit/delete the WAL, terminal ledger, binding ledger, pointer, or locks. `_state.json` is live/advisory; completion requires `_terminal.json`, manifest v2, terminal-ledger verification, and matching `name_binding_hash`. Process-crash recovery is qualified, not sudden-power-loss recovery. The retained supported runner record has no publication WAL.
