...
- Accepted and rejected hypotheses, summaries, risks, scores, and exact gate failures from up to six cycles across completed runs are injected into later prompts so the autonomous loop retains successful lessons and corrects prior regressions instead of blindly repeating them. Ignored disposable files are included in the raw-filesystem gate and must be written outside the worktree or removed before the agent finishes.
