...
First live run `20260711T223022Z-1128b831` failed closed before canary because CLI atomic-delete behavior exceeded the earlier ACL policy; it remains unanchored diagnostic evidence. Runner `20260712T005650Z` later failed before controller-run creation because stale-worktree preflight rejected its own live `.controller-v2.lock`. The scanner was corrected to exempt only that fixed lock entry. Exact BAT/CodexLightRunner DryRun runner `20260712T005801Z` then completed controller run `20260712T005807Z-709f67a4`: `DRY_RUN_COMPLETE`, container qualification and quiescence passed, ACL restored, production unchanged, evidence sealed and ledgered. DryRun did not execute candidate build/tests/train/score/reviews. That retained run predates v3.1 supervisor sealing; current qualification is 240 passed with 2 platform skips.
