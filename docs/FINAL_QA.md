...
Historical v2.1 QA does not qualify v2.2. `10/10` means all ten rubric dimensions received ten points, totaling 100/100, and human review only. Supported DryRun `20260712T005807Z-709f67a4` passed its preflight/isolation/restoration/protection evidence but intentionally scored 0/100 with execution/review `NOT_RUN`; it is not final QA. Current code qualification is 240 passed with 2 explicitly reported platform skips.
...
Runner v3.1 QA additionally requires manifest v2, a unique terminal-ledger anchor, valid hash-chain name binding and matching `name_binding_hash`, no pending publication WAL, and a matching authoritative terminal pointer. Advisory `_state` is not terminal evidence. Final qualification is 240 passed/2 skipped in 54.66 seconds; focused runner is 27 passed. The two skips explicitly report unavailable file- and directory-symlink creation on this host. Post-v3.1 supported DryRun `20260712T014742Z-2a8b7152` passed isolation, container/ACL quiescence, cleanup, and production protection, but remains 0/100 and `NOT_RUN`; it is not final candidate QA.
