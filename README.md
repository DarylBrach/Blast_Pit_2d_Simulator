...
The first v2 live run, `20260711T223022Z-1128b831`, failed closed before the sandbox canary because the CLI performed an atomic-delete operation that the earlier ACL policy denied; production remained unchanged and the unanchored bundle remains diagnostic only. Runner `20260712T005650Z` then failed before controller-run creation because stale-worktree scanning treated its own live `.controller-v2.lock` as unsafe. The scanner was corrected to recognize only that fixed controller-lock entry. The exact BAT/CodexLightRunner DryRun subsequently succeeded as runner `20260712T005801Z`, controller `20260712T005807Z-709f67a4`, with `DRY_RUN_COMPLETE`, unchanged production, passing container qualification/quiescence, restored ACL, sealed manifest, and ledger/latest publication. DryRun did not run candidate build, tests, training, scoring, specialist review, or final QA. That retained DryRun predates runner v3.1; current suite totals are recorded below.
...
CodexLightRunner v3.1 independently seals supervisor evidence with manifest v2 exact directory/file inventory, kernel locks, a persistent publication WAL, a terminal hash-chain ledger, and a separate hash-chain name-to-target binding ledger. `latest/<name>_terminal.json` is authoritative; `latest/<name>_state.json` is advisory/live. Startup recovers valid interrupted publication before run allocation. Each terminal-ledger row carries the matching `name_binding_hash`, and the terminal pointer binds that row by `ledger_hash`. These are process-crash-qualified local tamper controls, not signatures or sudden-power-loss guarantees. Local qualification is 241 passed/3 skipped in 54.51 seconds; governed-host live binding separately passes 1/1 with `BLAST_PIT_RUN_GOVERNED_HOST_TESTS=1`; focused runner remains 27 passed. Hosted CI performs mandatory static authorization and a capability-gated sanitized Windows ACL integration attempt but never synthesizes the exact Codex/Docker/image identity. Post-v3.1 DryRun runner `20260712T014740Z-04461e70` / controller `20260712T014742Z-2a8b7152` remains the retained operational record.
