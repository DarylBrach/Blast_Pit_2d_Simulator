...
The corrected ten-cycle v1.1 run completed with sealed evidence and no production changes, but final QA found that cycle 10 self-classified an algorithmic Hall-of-Fame change as `process`. That candidate is blocked; `de3be87` is the last pre-failure v1.1 accepted ancestor, has not been revalidated under v1.2, and remains pending human review. Controller v1.2 now treats every simulator-source change as algorithmic and routes normal disposable output to evidence-sealed cycle scratch space. The original v1.1 authorization is preserved; the launcher uses a separate v1.2 amendment. No candidate is merged, pushed, audited, released, or approved for production.
